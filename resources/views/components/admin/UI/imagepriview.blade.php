...
<div class="form-group" x-data="uploadImage()" x-init="init('{{ $image }}')">
  ...
  <div class="position-relative">
    ...
    <img id="image" class="rounded w-100" style="height: {{ $height }}; background-color: #e2e8f0; object-fit: contain;"
      :src="preview">
  ...
  <input type="file" name="{{$name}}" multiple x-ref="{{ $id ?? $name }}" name="{{$name}}" id="{{ $id ?? $name }}"
    required class="d-none {{ $class??'' }}" @change="updatePreview('{{ $id ?? $name }}')">
